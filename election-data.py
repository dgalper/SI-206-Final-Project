...
def enter_into_database(db_name, table_name ,year_winner_list, start_index, end_index):
    ...
    cur = conn.cursor()
    cur.execute(f'CREATE TABLE IF NOT EXISTS {table_name} (year TEXT PRIMARY KEY, winning_party TEXT)')
    while start_index <= end_index:
        year = list(year_winner_list[start_index])[0]
        print(year)
        winning_party = year_winner_list[start_index][year]
        print(winning_party)
        cur.execute(f'INSERT INTO {table_name} (year, winning_party) VALUES (?, ?)', (year, winning_party))
        start_index += 1
    conn.commit()
